...
class Ai1wm_Extractor extends Ai1wm_Archiver {
	...
	private function get_data_from_block( $block ) {
		...
		$format = array(
			$this->read_block_format[0] . 'filename/',
			$this->read_block_format[1] . 'size/',
			$this->read_block_format[2] . 'mtime/',
			$this->read_block_format[3] . 'path/',
			$this->read_block_format[4] . 'integrity',
		);
