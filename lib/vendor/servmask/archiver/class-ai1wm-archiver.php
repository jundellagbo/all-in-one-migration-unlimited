...
abstract class Ai1wm_Archiver {
	...
	/**
	 * Header block format used when reading a file
	 *
	 * All-in-One WP Migration 7.x reserves the last 8 bytes of every block for an
	 * integrity value, which leaves 4088 bytes for the path. Reading the trailing
	 * field separately is also correct for archives written by this version: their
	 * path field is NUL padded and a relative path can never reach 4088 bytes, so
	 * the last 8 bytes are always padding.
	 *
	 * Field Name    Offset    Length    Contents
	 * name               0       255    filename (no path, no slash)
	 * size             255        14    size of file contents
	 ...
	 * integrity       4369         8    integrity value (empty on older archives)
	 *
	 * @type array
	 */
	protected $read_block_format = array(
		'a255',  // filename
		'a14',   // size of file contents
		'a12',   // last time modified
		'a4088', // path
	...
	/**
	 * Check whether a block is the end of file block
	 *
	 * This version writes an all NUL block, while 7.x stores the archive size and an
	 * integrity value in it. Both leave the filename field empty, which never happens
	 * for a real file, so that is what we test.
	 *
	...
	protected function is_eof_block( $block ) {
		if ( strlen( $block ) !== strlen( $this->eof ) ) {
			return false;
		}

		return trim( substr( $block, 0, 255 ), "\0" ) === '';
	}
	...
	public function is_valid() {
		if ( ( $offset = @ftell( $this->file_handle ) ) !== false ) {
			if ( @fseek( $this->file_handle, -4377, SEEK_END ) !== -1 ) {
				if ( $this->is_eof_block( (string) @fread( $this->file_handle, 4377 ) ) ) {
					if ( @fseek( $this->file_handle, $offset, SEEK_SET ) !== -1 ) {
						return true;
					}
				}
