...
function ai1wm_storage_path( $params ) {
	...
	// The storage folder name is generated by us (see ai1wm_storage_folder), so it may only ever
	// be a plain alphanumeric token. Anything else is a caller trying to walk out of the storage
	// directory and is rejected outright rather than squeezed through basename().
	if ( ! ai1wm_validate_storage_name( $params['storage'] ) ) {
	...
	// Get storage path
	$storage = AI1WM_STORAGE_PATH . DIRECTORY_SEPARATOR . $params['storage'];
	if ( ! is_dir( $storage ) ) {
...
function ai1wm_backup_path( $params ) {
	...
	if ( ! ai1wm_validate_archive_name( $params['archive'] ) ) {
	...
	$path = AI1WM_BACKUPS_PATH . DIRECTORY_SEPARATOR . $params['archive'];

	// Belt and braces: the resolved path must still sit inside the backups directory
	if ( ! ai1wm_is_path_inside( AI1WM_BACKUPS_PATH, $path ) ) {
		throw new Ai1wm_Archive_Exception( __( 'Invalid archive path. <a href="https://help.servmask.com/knowledgebase/invalid-archive-path/" target="_blank">Technical details</a>', AI1WM_PLUGIN_NAME ) );
	}
...
function ai1wm_archive_path( $params ) {
	...
	if ( empty( $params['ai1wm_manual_restore'] ) ) {
		...
		// Archives living in the storage directory are always flat - a name with a directory
		// component there can only be an attempt to write outside of it.
		$path = $storage . DIRECTORY_SEPARATOR . basename( $params['archive'] );

		if ( ! ai1wm_is_path_inside( $storage, $path ) ) {
			throw new Ai1wm_Archive_Exception( __( 'Invalid archive path. <a href="https://help.servmask.com/knowledgebase/invalid-archive-path/" target="_blank">Technical details</a>', AI1WM_PLUGIN_NAME ) );
		}
...
function ai1wm_archive_file( $blog_id = null ) {
	...
	//
	// This used to be rand( 100, 999 ). Backup files live under a predictable URL, so those 900
	// combinations meant a whole site database was retrievable by anyone willing to spend a few
	// thousand requests guessing. Use a long cryptographically random suffix instead.
	$name[] = strtolower( wp_generate_password( 24, false, false ) );
...
function ai1wm_storage_folder() {
	// uniqid() alone is derived from the clock and is trivially guessable by anyone who can
	// observe roughly when an export started, so mix in cryptographic randomness.
	return uniqid() . strtolower( wp_generate_password( 16, false, false ) );
}
...
function ai1wm_verify_secret_key( $secret_key ) {
	$expected = ai1wm_secret_key();

	// hash_equals() keeps the comparison constant-time so the key cannot be recovered a byte at a
	// time by measuring how long a rejection takes.
	if ( ! is_string( $secret_key ) || $secret_key === '' || ! hash_equals( $expected, $secret_key ) ) {
...
/**
 * Get the secret key, creating or upgrading it when needed
 *
 * The secret key is the only thing standing between the internet and the export/import endpoints,
 * because those endpoints have to stay reachable by the plugin's own unauthenticated loopback
 * requests. Historically it was 12 characters; anything shorter than 32 is regenerated here.
 *
 * @return string
 */
function ai1wm_secret_key() {
	$secret_key = get_option( AI1WM_SECRET_KEY );

	if ( ! is_string( $secret_key ) || strlen( $secret_key ) < 32 ) {
...
/**
 * Verify that the caller is allowed to run a migration action
 *
 * Requests that carry a WordPress login must also hold the matching capability - a valid secret key
 * on its own must never let a subscriber export or overwrite the site. Requests without a login are
 * the plugin's own loopback calls and are authenticated by the secret key alone.
 *
 * @param  string       $secret_key   Secret key
 * @param  string|array $capabilities Capability, or list of capabilities of which one is enough
 * @return boolean
 * @throws Ai1wm_Not_Valid_Secret_Key_Exception
 */
function ai1wm_verify_access( $secret_key, $capabilities = 'import' ) {
	...
	if ( is_user_logged_in() ) {
		...
		if ( ! $permitted ) {
			throw new Ai1wm_Not_Valid_Secret_Key_Exception( __( 'You are not allowed to perform this action.', AI1WM_PLUGIN_NAME ) );
		}
	}
...
/**
 * Validate an archive name supplied by a request
 *
 * Accepts names such as "example-com-20200101-120000-abc.wpress" and, when subdirectories are
 * allowed, "folder/example-com-....wpress". Rejects traversal sequences, absolute paths, control
 * characters and anything that is not a .wpress archive.
 *
 * Users legitimately rename their backups, so the character rules stay permissive on purpose - the
 * security properties come from the traversal and extension checks, not from a narrow charset.
 *
 * @param  string  $archive              Archive name
 * @param  boolean $allow_subdirectories Whether a directory component is acceptable
 * @return boolean
 */
function ai1wm_validate_archive_name( $archive, $allow_subdirectories = true ) {
	if ( ! is_string( $archive ) || $archive === '' ) {
		return false;
	}

	// Normalise Windows separators so a single rule covers both platforms
	$archive = str_replace( '\\', '/', $archive );

	// No control characters (NUL in particular truncates the path for the C library underneath)
	if ( preg_match( '#[\x00-\x1F\x7F]#', $archive ) ) {
		return false;
	}

	// No absolute paths, no drive letters
	if ( substr( $archive, 0, 1 ) === '/' || preg_match( '#^[a-zA-Z]:#', $archive ) ) {
		return false;
	}

	if ( ! $allow_subdirectories && strpos( $archive, '/' ) !== false ) {
		return false;
	}

	// No traversal, and no empty segments
	foreach ( explode( '/', $archive ) as $segment ) {
		if ( $segment === '' || $segment === '.' || $segment === '..' ) {
			return false;
		}
	}

	// Must be a .wpress archive. This is the check that stops an "archive" being dropped into the
	// web-served storage directory as shell.php.
	if ( strtolower( substr( $archive, -7 ) ) !== '.wpress' ) {
		return false;
	}
...
/**
 * Validate a storage folder name supplied by a request
 *
 * @param  string  $storage Storage folder name
 * @return boolean
 */
function ai1wm_validate_storage_name( $storage ) {
	return is_string( $storage ) && preg_match( '#^[A-Za-z0-9]{8,64}$#', $storage ) === 1;
}

/**
 * Check that a path resolves inside a parent directory
 *
 * Resolves whichever leading portion of the path already exists, so it works for files that are
 * about to be created as well as ones already on disk.
 *
 * @param  string  $parent Parent directory
 * @param  string  $path   Path to test
 * @return boolean
 */
function ai1wm_is_path_inside( $parent, $path ) {
	if ( ! is_string( $parent ) || ! is_string( $path ) || $parent === '' || $path === '' ) {
		return false;
	}

	if ( strpos( $parent, "\0" ) !== false || strpos( $path, "\0" ) !== false ) {
		return false;
	}

	// Resolve the parent; if it does not exist there is nothing to contain the path
	if ( ( $real_parent = @realpath( $parent ) ) === false ) {
		return false;
	}

	// Walk up until we hit something that exists, then resolve that and re-attach the remainder
	$suffix = '';
	$target = $path;
	while ( ( $real_target = @realpath( $target ) ) === false ) {
		$dirname = dirname( $target );

		// dirname() stops changing at the filesystem root
		if ( $dirname === $target ) {
			return false;
		}
		...
		// A traversal segment in the not-yet-existing tail could still climb out once created
		if ( $segment === '..' || $segment === '.' ) {
			return false;
		}

		$suffix = DIRECTORY_SEPARATOR . $segment . $suffix;
		$target = $dirname;
	}

	$resolved    = $real_target . $suffix;
	$real_parent = rtrim( $real_parent, DIRECTORY_SEPARATOR );

	return $resolved === $real_parent || strpos( $resolved, $real_parent . DIRECTORY_SEPARATOR ) === 0;
}
